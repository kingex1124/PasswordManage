...
import { formatDateTime, fromDatetimeLocalValue, generateId, nowIsoString, toDatetimeLocalValue } from './utils.js';
...
function readRecordForm() {
  const passwordHistory = Array.from(elements.passwordHistoryEditor.children).map((itemElement) => {
    ...
    const changedAtInput = itemElement.querySelector('.history-date-input').value;
...
function refreshRecordFormMasterSelects(record = null) {
  const categories = PasswordRecordService.sortMasterItems(state.data.categories || []);
  const types = PasswordRecordService.sortMasterItems(state.data.types || []);

  fillRecordMasterSelect(elements.categoryInput, categories, record?.category || '');
...
function openRecordModal(record = null) {
  ...
  if (record) {
    ...
    record.passwordHistory.forEach((history) => {
      elements.passwordHistoryEditor.appendChild(createHistoryEditItem(history));
    });
  } else {
    elements.recordModalTitle.textContent = '新增資料';
    if ((state.data.categories || []).length === 0 || (state.data.types || []).length === 0) {
...
function renderMasterTable(masterType, bodyElement) {
  ...
  const items = PasswordRecordService.sortMasterItems(state.data[listKey] || []);
...
function updateFilterOptions() {
  const { categories, types } = PasswordRecordService.buildFilterOptions(state.data);
  state.filters.categories = state.filters.categories.filter((item) => categories.includes(item));
  state.filters.types = state.filters.types.filter((item) => types.includes(item));
  fillFilterSelect(elements.categoryFilter, categories, state.filters.categories);
...
function renderRecords() {
  ...
  records.forEach((record) => {
    ...
    row.children[0].textContent = record.category;
    ...
    record.passwordHistory.forEach((history) => {
      ...
      value.textContent = visible ? history.password : '•'.repeat(Math.max(6, history.password.length || 6));
      ...
      toggleBtn.addEventListener('click', () => togglePasswordVisibility(record.id, history.id));
...
async function handleImport(encryptedText, fileHandle = null) {
  ...
  try {
    const plainData = await encryptionService.decryptFilePayload(encryptedPayload, inputPassword);
...
async function handleExportClick() {
  try {
    const inputPassword = window.prompt('請輸入存檔密碼（可留空為目前密碼）', state.archivePassword || '');
    if (inputPassword === null) {
      return;
    }
    state.archivePassword = inputPassword;

    const encryptedPayload = await encryptionService.encryptRecords(state.data, state.archivePassword);
    const saveResult = await FileService.saveEncryptedPayload(encryptedPayload, state.importedFileHandle);
    ...
    if (saveResult.mode === 'overwrite') {
      ...
      setMessage('存檔成功，瀏覽器不支援原檔覆蓋，已改以下載方式匯出', 'success');
    }
  } catch {
...
function bindEvents() {
  ...
  elements.changeArchivePasswordBtn.addEventListener('click', () => {
    const newPassword = window.prompt('請輸入新的存檔密碼', state.archivePassword || '');
    if (newPassword === null) {
      return;
    }
  ...
  elements.addPasswordHistoryBtn.addEventListener('click', () => addHistoryEditorItem());
  ...
  elements.categoryFilter.addEventListener('change', () => {
    const selectedValues = Array.from(elements.categoryFilter.selectedOptions).map((item) => item.value);
    state.filters.categories = normalizeFilterSelection(selectedValues);
    fillFilterSelect(elements.categoryFilter, PasswordRecordService.buildFilterOptions(state.data).categories, state.filters.categories);
    renderRecords();
  });

  elements.typeFilter.addEventListener('change', () => {
    const selectedValues = Array.from(elements.typeFilter.selectedOptions).map((item) => item.value);
    state.filters.types = normalizeFilterSelection(selectedValues);
    fillFilterSelect(elements.typeFilter, PasswordRecordService.buildFilterOptions(state.data).types, state.filters.types);
    renderRecords();
  });
